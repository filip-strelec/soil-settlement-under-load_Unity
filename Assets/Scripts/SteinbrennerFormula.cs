...
double razlikaDubine = programState.dubinaZ- trenutnaDubinaZ;
   ...
   firstFraction = (double)Math.Atan((lengthL * widthB) / (trenutnaDubinaZ*Math.Sqrt(Math.Pow(lengthL,2) + Math.Pow(widthB,2)+ Math.Pow(trenutnaDubinaZ,2))));
    secondFraction = (double)(lengthL * widthB * trenutnaDubinaZ) / (Math.Sqrt(Math.Pow(lengthL, 2) + Math.Pow(widthB, 2) + Math.Pow(trenutnaDubinaZ, 2)));
